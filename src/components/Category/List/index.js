...
export default class List extends PureComponent {
   ...
   const query = genQueryArgs(this, offset, limit, querySchema);
     ...
     try {
      ...
      const {
        total,
        [entriesProp]: entries
      } = await this.fetchCategoryList(query);
   ...
   *  激活已选择条目
   */
  publishEntries = () => handleEntries(this, '激活', 'publishCategory', {
   ...
   *  恢复已选择条目
   */
  recoverEntries = () => handleEntries(this, '恢复', 'recoverCategory', {
   ...
   *  冻结已选择条目
   */
  blockEntries = () => handleEntries(this, '冻结', 'blockCategory', {
    shouldIgnore: entry => !entry.publishAt
  });
   ...
   *  删除已选择条目
   */
  destroyEntries = () => handleEntries(this, '删除', 'destroyCategory', {
    shouldIgnore: entry => entry.removeAt
  });
