...
export default class List extends PureComponent {
   ...
   const query = genQueryArgs(this, offset, limit, querySchema);
     ...
     try {
      ...
      const {
        total,
        [entriesProp]: entries
      } = await this.fetchVersionList(query);
   ...
   *  激活已选择条目
   */
  publishEntries = () => handleEntries(this, '激活', 'publishVersion', {
   ...
   *  恢复已选择条目
   */
  recoverEntries = () => handleEntries(this, '恢复', 'recoverVersion', {
   ...
   *  冻结已选择条目
   */
  blockEntries = () => handleEntries(this, '冻结', 'blockVersion', {
    shouldIgnore: entry => !entry.publishAt
  });
   ...
   *  删除已选择条目
   */
  destroyEntries = () => handleEntries(this, '删除', 'destroyVersion', {
    shouldIgnore: entry => entry.removeAt
  });
