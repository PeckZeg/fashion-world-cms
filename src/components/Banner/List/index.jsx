...
export default class List extends PureComponent {
   ...
   const query = genQueryArgs(this, offset, limit, querySchema);
     ...
     try {
      ...
      const {
        total,
        [entriesProp]: entries
      } = await this.fetchBannerList(query);
   ...
   */
  publishEntry = btn => handleEntry(this, btn, '发布', 'publishBanner');

  /**
   *  恢复条目
   *  @param {React.Component} button 按钮组件实例
   */
  recoverEntry = btn => handleEntry(this, btn, '恢复', 'recoverBanner');

  /**
   *  冻结条目
   *  @param {React.Component} button 按钮组件实例
   */
  blockEntry = btn => handleEntry(this, btn, '冻结', 'blockBanner');

  /**
   *  删除条目
   *  @param {React.Component} button 按钮组件实例
   */
  destroyEntry = btn => handleEntry(this, btn, '删除', 'destroyBanner');
  ...
  publishTimingEntry = async (modal, form, entry) => {
    ...
    try {
      ...
      const { [entryProp]: newEntry } = await this.publishBanner(entryId, body);
   ...
   *  激活已选择条目
   */
  publishEntries = () => handleEntries(this, '激活', 'publishBanner', {
   ...
   *  恢复已选择条目
   */
  recoverEntries = () => handleEntries(this, '恢复', 'recoverBanner', {
   ...
   *  冻结已选择条目
   */
  blockEntries = () => handleEntries(this, '冻结', 'blockBanner', {
    shouldIgnore: entry => !entry.publishAt
  });
   ...
   *  删除已选择条目
   */
  destroyEntries = () => handleEntries(this, '删除', 'destroyBanner', {
    shouldIgnore: entry => entry.removeAt
  });
